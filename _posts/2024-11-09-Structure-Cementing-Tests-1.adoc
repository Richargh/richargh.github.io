...
First we introduce the TestState (<<lst:testdsl-teststate>>).

[[lst:testdsl-teststate]]
.TestState of the TestDsl
[source,java]
----
...
----
<1> The TestState is a class that recognizes all builders.
...
To ensure that the tests are isolated from each other, we instantiate TestState and Floor anew for each test (<<lst:testdsl-init-before-each>>).
...
Our test already looks very compact (<<lst:testdsl-complete-test-with-before-each>>).
